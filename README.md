...
- Thread exit: Assembly code in RTX causes threads to call `osThreadExit()` after they return from their main functions.  RTXOff cannot do this so any thread must call `osThreadExit()` directly instead of returning from its main function. 
