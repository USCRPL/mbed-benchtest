...
static void test_equeue_period()
{
    ...
    // RTXOff: made this a little more lenient
    //TEST_ASSERT_EQUAL_UINT8(5, touched);
	TEST_ASSERT_UINT_WITHIN(2, 5, touched);
...
static void test_equeue_multithread()
{
    ...
    t1.start(mbed::callback(multithread_thread, &q));
    ThisThread::sleep_for(10ms);
    equeue_break(&q);
    err = t1.join();
    TEST_ASSERT_EQUAL_INT(0, err);
...
static void test_equeue_multithreaded_barrage()
{
    ...
    t1.start(mbed::callback(ethread_dispatch, &t));
