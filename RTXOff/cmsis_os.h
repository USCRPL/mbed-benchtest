...
#define osKernelSystemId   "RTXOff V0.1"   ///< RTOS identification string
...
#else
#define osThreadDef(name, priority, stacksz) \
uint64_t os_thread_stack##name[(stacksz)?(((stacksz+7)/8)):1]; \
static osRtxThread_t os_thread_cb_##name; \
const osThreadDef_t os_thread_def_##name = \
{ (name), \
...
#else
#define osTimerDef(name, function) \
static osRtxTimer_t os_timer_cb_##name; \
...
#else
#define osMutexDef(name) \
static osRtxMutex_t os_mutex_cb_##name; \
...
#else
#define osSemaphoreDef(name) \
static osRtxSemaphore_t os_semaphore_cb_##name; \
...
#else
#define osPoolDef(name, no, type) \
static osRtxMemoryPool_t os_mp_cb_##name; \
static uint32_t os_mp_data_##name[osRtxMemoryPoolMemSize((no),sizeof(type))/4]; \
const osPoolDef_t os_pool_def_##name = \
{ (no), sizeof(type), \
...
#else
#define osMessageQDef(name, queue_sz, type) \
static osRtxMessageQueue_t os_mq_cb_##name; \
static uint32_t os_mq_data_##name[osRtxMessageQueueMemSize((queue_sz),sizeof(uint32_t))/4]; \
const osMessageQDef_t os_messageQ_def_##name = \
{ (queue_sz), \
...
static osRtxMemoryPool_t   os_mail_mp_cb_##name; \
static osRtxMessageQueue_t os_mail_mq_cb_##name; \
static uint32_t os_mail_mp_data_##name[osRtxMemoryPoolMemSize  ((queue_sz),sizeof(type) )/4]; \
static uint32_t os_mail_mq_data_##name[osRtxMessageQueueMemSize((queue_sz),sizeof(void*))/4]; \
const osMailQDef_t os_mailQ_def_##name = \
